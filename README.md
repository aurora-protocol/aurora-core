...
- policy profiles, PAL scoring, PACE reference behavior, local config parsing, and threat-safe logging wrappers;
- client engine: a runnable Linux local proxy client with HTTP CONNECT and SOCKS5 TCP/UDP-association listeners, client FLOW_OPEN frame emission, fake-IP mapped UDP flow integration, UDP target confirm TTL enforcement, synthetic and negative-cache-aware local DNS forwarder responses, and packet-to-core platform ABI forwarding;
- carriers and transport: cover-template-validated H2/H1/shadow-origin/H3 datagram and explicitly gated MASQUE carrier request builders, shared opaque carrier session adapters, and platform adapter conformance profiles;
- relay and server: relay frame-block flow demux, append-only file replay cache, relay admission/exit gates, an HTTP cover-origin gateway handler, a runnable Linux server harness, and a cover-origin deployment conformance harness;
- issuance: a Privacy Pass Blind RSA production proof harness, private proof-type validation gates, an issuer operations conformance harness, issuer service and HTTP lab-handler readiness harnesses, a loopback-only gateway-mTLS Blind-RSA production backend, and a binary issuer verifier mTLS handler;
- evaluation and evidence: a protocol decode fuzz harness, a gateway-backed active-probe harness, a deterministic DPI/classifier baseline harness, and external evaluation, deployment security assessment, and release readiness evidence verifiers;
- conformance and release gates: P0 host build matrix checks, P5 split-route and P6 proxy-flow conformance harnesses, a live HTTP/HTTPS server-client interop harness, vector drift tests, the P9 measured cover production-candidate threshold gate, the P10 deterministic performance/bad-path impairment harness (loss/jitter/UDP-blocked/QUIC-blocked/HoL/overload/peak-hour/carrier-cache scenarios plus a wall-clock packet-protect throughput regression floor, with interactive-priority, UDP stale-drop, reconnect-storm-free downgrade, block-suspect cooldown, and padding-under-congestion acceptance), the P11 prototype-interop release-gate checklist, and the aggregate `p0-p11-check`;
- `auroractl` diagnostics, a platform packaging and entitlement conformance matrix, and ops helpers.
...
- `cmd/aurorad`: Linux relay daemon (`serve`), private issuer backend (`issuer`), and local diagnostic harness.
- `cmd/auroralab`: local lab testing only — mints and serves disposable self-signed lab deployments for end-to-end device testing; never production infrastructure. See the [lab testing guide](docs/operations/auroralab-lab-testing.md).
- `docs/operations`: operator guides; `docs/superpowers`: dated design plans and specs behind larger changes.
