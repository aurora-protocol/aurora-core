...
func NewRetentionFileReplayCacheAt(directory *os.File, name string, nowUnix uint64) (*RetentionFileReplayCache, error) {
	...
	if err := cache.withLock(func() error { return cache.load(nowUnix) }); err != nil {
		_ = lock.Close()
		return nil, err
	}
	return cache, nil
...
func (c *RetentionFileReplayCache) load(nowUnix uint64) error {
	...
	for scanner.Scan() {
		...
		if len(fields) > 2 || len(fields) == 2 && fields[1] == "" {
			return fmt.Errorf("admission: retention replay cache %s has malformed entry", c.path)
		}
		key, err := hex.DecodeString(fields[0])
		if err != nil || len(key) == 0 || hex.EncodeToString(key) != fields[0] {
			return fmt.Errorf("admission: retention replay cache %s has malformed key", c.path)
		}
		...
		if len(fields) == 2 {
			...
			if err != nil || deadline == 0 || strconv.FormatUint(deadline, 10) != fields[1] {
				return fmt.Errorf("admission: retention replay cache %s has malformed retention deadline", c.path)
			}
		}
		if deadline > nowUnix {
...
func (c *RetentionFileReplayCache) rewrite() error {
	temporary, err := os.CreateTemp(c.directory.Name(), "."+c.name+".compact-")
	if err != nil {
		return err
	}
