...
// — the nil flowsByID map returns nil at :799; mapping == nil short-circuits :800
// before mapping.peerClosed; :801 returns the unique error.
//
...
func TestPacketAdapterHandleRelayDataLockedUnknownFlowGuard(t *testing.T) {
	...
	// short-circuits the compound :800 guard before mapping.peerClosed (nil
	// deref panic); :801 returns "data for unknown flow".
	_, err := (&PacketAdapter{}).handleRelayDataLocked(protocol.AuroraFrame{FlowID: 1})
	if err == nil {
		t.Fatal("handleRelayDataLocked(unknown flow) err = nil, want non-nil (:801)")
	}
	if !strings.Contains(err.Error(), "data for unknown flow") {
		t.Fatalf("unknown-flow err = %q, want \"...data for unknown flow\" (:801)", err.Error())
	}
}
