...
// AppendVarint appends the canonical variable-length encoding of v to dst.
func AppendVarint(dst []byte, v uint64) ([]byte, error) {
	if v > MaxVarint {
		return dst, fmt.Errorf("wire: varint out of range: %d", v)
	}
	switch {
	case v <= 63:
	...
	case v <= 16383:
		x := uint16(v) | 0x4000
		return append(dst, byte(x>>8), byte(x)), nil
	case v <= 1073741823:
		x := uint32(v) | 0x80000000
		return append(dst, byte(x>>24), byte(x>>16), byte(x>>8), byte(x)), nil
	default:
